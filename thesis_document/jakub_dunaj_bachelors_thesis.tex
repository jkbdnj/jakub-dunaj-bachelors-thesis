...
%----------------------------------------------------------------------------------------
\begin{abstract}
    The field of automatic plant disease classification poses a very interesting area for the application of artificial intelligence. A deep neural network could be trained to recognize and classify plant diseases from leaf images. This application of deep neural networks may indeed have very positive impact on humans and ecosystems. This bachelor's thesis focuses on the use of convolutional neural networks (CNNs). Many approaches have already been made to train a CNN for the task of recognition and classification of plant diseases from leaf images. This thesis reviews recent and most influential work that has been done in this field so far. The thesis then applies the gained information to train a CNN model. The design of the dataset, the CNN architecture chosen, and the training and validation strategy are presented in greater detail. The developed code used for the training process of the CNN model is also introduced. Eventually, the thesis reviews the performance of the trained CNN and draws a conclusion.
\end{abstract}
...
The research objective of this bachelor's thesis is the plant disease classification from leaf images using artificial intelligence. The ability to recognize and classify plant diseases from plant images has different very important implications and impacts. Being able to classify a disease at an early stage may save substantial part of the crops. This saves the yield and considerably reduces the money loss. The early disease recognition and classification may also reduce the amount of pesticides applied (\cite{Sladojevic:2016}). The pesticides are toxic, and their use poses direct negative impact on environment and has serious effects on human health (\cite{Aktar:2009}). Moreover, the pesticides contaminate surface water, groundwater, and soil (\cite{Aktar:2009}). The pesticides also contaminate beneficial non-target organisms, such as insects or other plants (\cite{Aktar:2009}). This means that the use of pesticides has severe ecological and sociological consequences. The early disease recognition and classification may also increase the food security in less developed countries (\cite{Mohanty:2016}).

This thesis focuses on plant disease classification from leaf images using deep neural network. IN this thesis, convolutional neural networks (CNNs) were used. In \autoref{sub:machine-learning-overview}, the \autoref{sec:thesis-focus-and-related-research} gives an overview about machine learning and neural networks. In \autoref{sec:thesis-focus-and-related-research} the related research in the field of plant disease classification using CNNs is reviewed. In \autoref{sub:dataset}, the \autoref{sec:design} describes the dataset that will be used to train a CNN. Subsection \ref{sub:cnn-architecture} describes the CNN architecture that will be trained. Subsection \ref{sub:training-and-validation} proposes the training and validation strategy applied during the training process of the CNN model. Section \ref{sec:development} introduces and explains the developed code that is used to train the CNN model. For the development of the code, Keras and TensoFlow Python libraries have been used. Section \ref{sec:discussion} reviews the performance of the model during the training process and presents the future work. Eventually, the \ref{sec:conclusion} summarizes this bachelor's thesis and its impact.

\section{Thesis Focus and Related Research}
\label{sec:thesis-focus-and-related-research}
    ...
    \label{sub:machine-learning-overview}
...
\label{sec:discussion}
    In this section, the performance of the trained EfficientNetB0 model during the training process will be evaluated. Furthermore, the future work and the possible applications of the trained model will be discussed.

    \subsection{CNN Model Performance Review}
        The training process was divided into two cycles: the initial and the final training cycles. During the initial cycle, only the last dense classifier layer of the model with 38 outputs was trained. Other layers or trainable parameters were kept frozen. In the final cycle, all trainable parameters of the model were trained. Each of these two training cycles had 8 epochs. After each epoch, the model was tested using 20\% of the train subset of the final dataset. After the whole training cycle, the model was evaluated using the validation subset of the final dataset. Moreover, after each training cycle, the average train and test accuracy and loss and the train and test accuracy per class were saved as plots. The train and test accuracy per class metrics were averaged over all epochs of the training cycle for each class. The train and test accuracy metrics were averaged over all classes in one epoch. Additionally, the evaluation data and the whole training history for a training cycle were saved as JSON files. The whole training process was done on a personal computer and took 4 hours and 10 minutes to complete. There was also an attempt made to train the model using Google Colab. Unfortunately, because of network limitations, this approach was impractical and would have taken substantially longer.

        \autoref{fig:initial-accuracy-and-loss} shows the accuracy and loss metrics of the model during the whole initial training cycle. The model achieved a train accuracy of 85.5\% and a test accuracy of almost 90\% in the last epoch. The train loss was 0.56 and the test loss was 0.35 in the last epoch.
        ...
        \autoref{fig:initial-train-accuracy-per-class} shows accuracies of each class averaged over all 8 epochs during the actual training in the initial cycle. The model reached 83\% average accuracy over all classes. The model reported an accuracy of at least 80\% in 71\% of the classes. In 34\% of the classes, the model reached an accuracy of at least 90\%. \autoref{fig:initial-train-accuracy-per-class} also shows that the model reported lower accuracies in tomato disease classes. Looking at the average per class metric in each epoch, the model struggled to correctly predict the tomato disease classes in early epochs. In the last epoch, the model achieved an accuracy of at least 62\% for each tomato disease class during the actual training. The average accuracy of the model in tomato disease classes was 70.2\%. The revision of the images in the tomato disease classes showed that these images were very similar in terms of disease symptoms. The model would need more images to learn to distinguish between the different tomato leaf diseases and classify these reliably when compared to other dataset classes.
        \begin{figure}[!h]
            \centering
        ...
        \autoref{fig:initial-test-accuracy-per-class} shows accuracies of each class averages over all 8 epochs during testing in the initial cycle. The average accuracy over all classes was 88\%. The model reported very similar accuracies to those achieved during the training. Same as in the training phase, the model reported lower accuracies in the case of tomato disease classes. Unfortunately, the model struggled in the last epoch and could not reliably predict certain tomato disease classes. The average accuracy of tomato disease classes was 76\%. After the initial training cycle, the model was evaluated and reported an average accuracy of 89.7\% and a loss of 0.37.
        ...
        \autoref{fig:final-accuracy-and-loss} shows the accuracy and loss of the model during the final training cycle. The model achieved a train accuracy of 92.8\% and a test accuracy of 88.5\% in the last epoch. The train loss was 0.22 and the test loss was 0.39. \autoref{fig:final-accuracy-and-loss} also shows a very interesting decrease of the model's accuracy and increase of the loss in the fourth epoch.
        \begin{figure}[!h]
            \centering
        ...
        The final training cycle applied the fine-tuning step. That is why the \autoref{fig:final-train-accuracy-per-class} and \autoref{fig:final-test-accuracy-per-class} report slightly worse results as the \autoref{fig:initial-test-accuracy-per-class} and \autoref{fig:final-accuracy-and-loss}. \autoref{fig:final-train-accuracy-per-class} and \autoref{fig:final-test-accuracy-per-class} show that the model struggled to correctly predict the tomato disease classes as in the initial cycle. Moreover, the \autoref{fig:final-test-accuracy-per-class} shows that the model's performance during the testing was not optimal.
        \begin{figure}[!h]
            \centering
        ...
        \end{figure}
        The evaluation of the final model after the final training cycle reported an accuracy of 89\% and a loss of 0.38.

        The validation accuracies and losses of the model after each training cycle are considerably high. But after looking at the accuracy per class metric either as averaged value over all epochs or within single epoch, the model struggled to reliably predict the tomato disease classes. The average accuracy per class metric does not express the actual performance of the model because it also takes the accuracies from earlier epochs. In these epochs, the model will be always less accurate than in the later epochs. The metric also depends on the number of epochs done. However, the average accuracy per class metric shows the model's consistency in each class. In this case, the metric clearly shows that the model is not consistent over all classes and cannot classify tomato disease classes with such high accuracy as the other classes. Using a dataset that would contain more images in the tomato disease classes would have helped the model to learn to distinguish between the classes. Moreover, to achieve even higher accuracies, the model would need more images to learn from. However, given the dataset and computing limitations, the model reported considerably high accuracies. This is highlighted by the fact that \textcite{Tan:2019} trained the EfficientNetB0 on ImageNet dataset and reported the average accuracy of 77.1\%.

    \subsection{Future Work}

        The review of model's performance during the training process shows that the model struggled to accurately classify tomato disease images. It also shows that the model's overall performance can be increased. The next step would be to extend the dataset and apply more epochs. \autoref{fig:initial-accuracy-and-loss} and \autoref{fig:final-accuracy-and-loss} show that the losses of the model converged rather quickly and stayed relatively high. This also shows that the dataset has to contain more images. Moreover, using the same dataset for training, testing and validation may result in inflated accuracies as shown by \textcite{Mohanty:2016}. The next step would be to evaluate the model on new and unseen images. The obvious limitation of the dataset is that it does not contain images taken under complex field conditions. To make the model generalizable, the dataset would have to contain even grater variety and variability of images.

        Despite the dataset limitations, the EfficientNetB0 architecture is efficient and contains rather small number of parameters. It can be trained very easily on a personal computer with high number of images. It was explicitly designed for edge devices and mobile phones that have limited computational resources. An EfficientNetB0 model that would generalize sufficiently can be embedded into a mobile application or a drone. Using a drone to examine crops on the field and search for potential diseases could considerably lower expenses. Large fields could be checked and an emerging disease could be spot early on. This would lower the use of pesticides or insecticides and eliminate potential losses.

\section{Conclusion}
\label{sec:conclusion}

    During this bachelor's thesis, plant disease classification using convolutional neural networks is explored. At the beginning, the thesis gives general information about machine learning and different machine learning approaches. Further it focuses on deep neural networks and especially on convolutional neural networks. It gives insights into the architecture of deep neural networks and how they learn. It also explores different factors that may negatively influence the training process and the performance of a deep neural network. Furthermore, the thesis explores and reviews related and influential research papers done in the field of plant disease classification using CNNs. The review of the related research focuses on the impact of the dataset size, its variety and variability to model's performance. This review gives also insights into the training process of a CNN architecture. Building on the information about deep neural networks and especially CNNs and on the information gained from the related research, an approach to train a CNN model was done. This thesis describes how the training process was designed and how the needed code to train a CNN model was developed. Eventually, the thesis reviews the performance of the trained model.

    The biggest limitation of the training process was the dataset used. There are no freely available datasets that could be used to train a robust CNN model that could be used in field conditions. The dataset used in this thesis lacks the variability in the images. To introduce more variability into the dataset, the training process uses an augmentation step and adds artificial backgrounds to segmented leaf images. Additionally, it uses a preprocessing that for example randomly rotates or flips the images. The trained model reported very optimistic results. However, larger dataset would be required to increase mode's accuracy. To make the model generalize well, the dataset would also need images form field conditions.

    It is very hard to train a CNN model that could generalize well on new and unseen plant leaf images. It requires a robust dataset that contains images taken under various conditions in the field. Images taken in the field are very complex. The images may contain other plants, different plant parts, or different lighting conditions. Leaves may also have different orientations. The images may be taken during different seasons. Moreover, plant diseases may show different symptoms during the different developmental stages. All this makes the task of plant disease classification using convolutional neural networks difficult. However, different preprocessing techniques could be applied to make the dataset more robust. Additionally, the prediction pipeline could contain more steps. It could recognize leaves on images, crop these, and preprocess the color before making the prediction.

    In conclusion, this bachelor's thesis gives great insights into the field of plant disease classification using convolutional neural networks. It makes an overview of machine learning and reviews related research in plant disease classification using CNNs. Moreover, the information gained from the theoretical part of this thesis is used to train a CNN to classify plant diseases from leaf images.
