...
\section{Design}
    \subsection{Dataset}
    % There are not many free datasets available on the Internet. Only Mohanty and Barbedo. As of today barbedo barbedo's dataset is relatively small small
    % using color and segmented versions of the Mohanty dataset based on the PlantVillage dataset and then crating a dataset with artificial background from the segmented dataset
    % dividing the dataset into training/validation and testing dataset using hold-out method and then dividing the trainig/validation dataset into training and validation datasets using the 10-fold cross validation technique
    % using augmentation: 
    \subsection{CNN Model} % EfficientB0 (from pretrained models in Keras)
