...
\section{Design}
    ...
    \label{sub:trainig-and-validation}
...
\section{Discussion}
    ...
    \subsection{Future Work}
        ...
        The review of model's performance during the training process shows that the model struggled to accurately classify tomato disease images. It also shows that the model's overall performance can be increased. The next step would be to extend the dataset and apply more epochs. \autoref{fig:initial-accuracy-and-loss} and \autoref{fig:final-accuracy-and-loss} show that the losses of the model converged rather quickly and stayed relatively high. This also shows that the dataset has to contain more images. Moreover, using the same dataset for training, testing and validation may result in inflated accuracies as shown by \textcite{Mohanty:2016}. The next step would be to evaluate the model on new and unseen images. To make the model generalizable, the dataset would have to contain even grater variety and variability of images. The obvious limitation of the dataset is that it does not contain images taken under complex field conditions.

        Despite the dataset limitations, the EfficientNetB0 architecture is efficient and contains rather small number of parameters. It can very efficiently be trained on a personal computer with high number of images. It was explicitly designed for edge devices and mobile phones that have limited computational resources. A EfficientNetB0 model that would generalize sufficiently can be easily embedded into a mobile application or a drone. Using a drone to examine crops on the field and search for potential diseases could considerably lower expenses. Large fields could be checked and an emerging disease could be spot early on. This would lower the use of pesticides or insecticides and eliminate potential losses.
...
\section{Conclusion}
    ...
    It is very hard to train a CNN model that could generalize well on new and unseen plant leaf images. It requires a robust dataset that contains images taken under various conditions in the field. Images taken in the field are very complex. The images may contain other plants, different plant parts, or different lighting conditions. Leaves my also have different orientations. The images may be taken during different seasons. Moreover, plant diseases may show different symptoms during the different developmental stages. All this makes the task of plant disease classification using convolutional neural networks difficult. However, different preprocessing techniques could be applied to make the dataset more robust. Additionally, the prediction pipeline could contain more steps. It could recognize leaves on images, crop these, and preprocess the color before making the prediction.
